...
#include "libFace3d/3dFaceModel01/MM_Restricted/Definitions.h"
#include "libFace3d/3dFaceModel01/MM_Restricted/DataContainer.h"
#include "libFace3d/3dFaceModel01/MM_Restricted/MultilinearModel.h"
...
public:
	...
	void reconstructForVariations(const std::vector<double>& variations, std::vector<float>& outPoints);
	...
	//! \param outPoints			vertices of the reconstruction
	void reconstructForWeights(const std::vector<double>& weightVector, std::vector<float>& outPoints);
	void reconstructForWeights(const double* weightVector, std::vector<float>& outPoints);

	//! Get the vertices of the learned data mean.
	//! \param dataMean			learned data mean
	void getDataMeanVertices(std::vector<float>& dataMean);

	//! Get the vertices of the learned neutral data mean.
	//! \param neutralDataMean	learned neutral data mean
	void getNeutralDataMeanVertices(std::vector<float>& neutralDataMean);
	...
	size_t getWeightsSize() const { return m_meanWeights.size(); }
	
	//! return neutral mean weights
	const std::vector<double> &getNeutralMeanWeights() const {
		return m_neutralMeanWeights;
	}
