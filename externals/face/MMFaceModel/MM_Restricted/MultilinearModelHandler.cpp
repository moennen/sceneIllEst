...
#include "libFace3d/3dFaceModel01/MM_Restricted/KDTree3.h"
...
namespace 
{
	void getPlaneProjection(const Vec3d& p1, const Vec3d& p2, const Vec3d& n2, Vec3d& outPoint)
	{
		Vec3d diff = p2-p1;
...
void MultilinearModelHandler::reconstructForVariations(const std::vector<double>& variations, std::vector<float>& outPoints)
{
...
void MultilinearModelHandler::reconstructForWeights(const std::vector<double>& weightVector, std::vector<float>& outPoints)
{
	const size_t m2 = m_truncModeDimensions[1];
	const size_t m3 = m_truncModeDimensions[2];
	...
	reconstructForWeights(&weightVector[0],outPoints);
}

void MultilinearModelHandler::reconstructForWeights(const double *weightVector, std::vector<float>& outPoints)
{
	if(m_truncModeDimensions.size()!=3)
	{
		std::cout << "Wrong number of mode diemensions " << m_truncModeDimensions.size() << " != " << 3 << std::endl;
...
void MultilinearModelHandler::getDataMeanVertices(std::vector<float>& dataMean)
{
	reconstructForWeights(m_meanWeights, dataMean);
}

void MultilinearModelHandler::getNeutralDataMeanVertices(std::vector<float>& neutralDataMean)
{
	...
	for(size_t i = 0; i < numSourceVertices; ++i)
	{
		...
		if(bPointValid)
		{
			...
			if(bHasValidNormals)
			{
				...
				getPlaneProjection(sourcePoint, nnPoint, targetNormal, planeProjectionPoint);
