...
public :
   Face3dMLModelTracker(const char *faceMeanModelFilename, 
                        const char *faceMLModelFilename);
   virtual ~Face3dMLModelTracker();
   
   bool initialized() const {return _modelFaces.size();}
   
   bool getTransformFromLandmarks(const TFaceLandmarks&,
                                  TFaceTransform &);
   bool getMeanNeutralParams(TFaceParams &) const; 
   bool getNeutralParamsFromLandmarks(const TFaceLandmarks&,
                                      TFaceParams &);
   
   bool updateFaceTexture( const TImg &img,
                           const TFaceTransform &,
                           const TFaceParams &,
                           float alpha,
                           TImg &faceTexture );
                           
   bool getVerticesFromParams(const TFaceParams &, 
                              std::vector<float> &);
 ...
 protected :
     ...
     static const size_t _landmarksIdx[13];
};
