...
struct Sampler final
{
   ...
   {
      if (buffId == 0) return 3;
      else if (buffId == 2) return 1;
      else return 2;
   }
   ...
   bool sample( float* buff )
   {
      ...
      for ( size_t s = 0; s < _sampleSz.x; ++s )
      {
         ...
         Mat normalsSple( _sampleSz.z, _sampleSz.y, CV_32FC2, currBuffNormals );
         processNormals( currNormals, normalsSple );

         currBuffImg += imgBuffSz;
         currBuffDepth += depthBuffSz;
         currBuffUVs += uvBuffSz;
         currBuffNormals += uvBuffSz;
      }
...
#pragma omp parallel for
      for ( size_t y = 0; y < in.rows; y++ )
      {
         ...
         for ( size_t x = 0; x < in.cols; x++ )
         {
            ...
            out_data[x] = vec2( norm.x, norm.y ) / norm.z;
         }
      }
   }
};
