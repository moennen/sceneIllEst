...
template <unsigned InDim = 2, unsigned OutDim = InDim, unsigned Order = 2, typename Real = float>
class PhSpline final
{
  ...
  private:
   ...
   static inline void
   fit( const Real* ctrlPts, const Real* ctrlFuncs, const size_t nCtrlPts, Real* params )
   {
      using namespace Eigen;
      // system matrix
      // -->
